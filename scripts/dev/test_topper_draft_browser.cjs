...
(async()=>{
 ...
 try{
  ...
  const reveal=async selector=>{const indices=await page.locator(selector).evaluate(el=>{const ds=[...document.querySelectorAll('#topperCard details')],list=[];for(let p=el.parentElement;p;p=p.parentElement)if(p.tagName==='DETAILS'&&!p.open)list.unshift(ds.indexOf(p));return list;});for(const i of indices)await page.locator('#topperCard details').nth(i).locator(':scope > summary').click();};
  ...
  await reveal('#tpArtRotation');await page.locator('#tpArtRotation').fill('90');await page.locator('#tpArtRotation').dispatchEvent('input');
  ...
  await reveal('#tpArtSize');assert.equal(await page.locator('#tpSave').isEnabled(),true);assert.match(await page.locator('#tpArtSize').innerText(),/^\d+\.\d+ × \d+\.\d+ × \d+\.\d+ mm$/);
  ...
  await reveal('#tpExport');const downloadP=page.waitForEvent('download');await page.locator('#tpExport').click();const download=await downloadP,stream=await download.createReadStream(),chunks=[];for await(const chunk of stream)chunks.push(chunk);
